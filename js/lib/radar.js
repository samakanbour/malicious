var RadarChart = {
	draw: function(id, d, options) {
		...
		g.call(tip);
		d.forEach(function(y, x) {
			g.selectAll(".nodes").data(y).enter().append("svg:circle").attr("class", "radar-chart-serie" + series).attr('r', cfg.radius)
				...
				.attr("cx", function(j, i) {
					...
					return j.axis
				}).style("fill", cfg.color(series)).style("fill-opacity", .9).style("cursor", "pointer").on('mouseover', tip.show).on('mouseout', tip.hide);
			series++;
		});
	}
};